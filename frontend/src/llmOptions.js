// Model options for the LLM selectors. Base list is static; custom models
// (an NVIDIA id from build.nvidia.com, an OpenRouter id from openrouter.ai/models)
// are user-managed in Settings and mirrored to localStorage so both selectors
// pick them up without a fetch.
...
export const BASE_LLM_OPTIONS = [
  ...
  { value:'openrouter', label:'OpenRouter (Free tier · cheap paid fallback)' },
...
// Full option list = base + custom models (value "<provider>:<model-id>").
// ponytail: entries saved before this had no provider field — they were all NVIDIA.
export function getLlmOptions() {
  const custom = getCustomNvidiaModels()
    .filter(m => m && m.id)
    .map(m => ({ value: `${m.provider || 'nvidia'}:${m.id}`, label: `⚡ ${m.label || m.id}` }))
